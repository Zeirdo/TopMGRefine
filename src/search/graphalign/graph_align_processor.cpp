  ...
  return [mng_ptr, var_mod_ptr_vec, spectrum_num, idx]() {
    ...
    while (spec_set_ptr != nullptr) {
      ...
      if (spec_set_ptr->isValid()) {      
        ...
        if(!mng_ptr->all_spectrum_ && !mng_ptr->all_protein_){
          if(spec_id == mng_ptr->specific_spectrum_id_){
            if (selected_prsm_ptrs.size() > 0) {
              ...
              for (size_t i = 0; i < selected_prsm_ptrs.size(); i++) {
                ...
                if(seq_name == mng_ptr->specific_protein_name_){
        ...
        else if(!mng_ptr->all_spectrum_ && mng_ptr->all_protein_){
          if(spec_id == mng_ptr->specific_spectrum_id_){
            if (selected_prsm_ptrs.size() > 0) {
              SpecGraphPtrVec spec_ptr_vec
                  = spec_reader.getNextSpecGraphPtrVec(spec_set_ptr, mng_ptr->prec_error_);
...
void GraphAlignProcessor::process() {
  ...
  std::cout << "spectrum_num: " << spectrum_num << std::endl; 
  std::string input_file_name
  ...
  //int exist = access("database.fasta_target_0", 0);
  //std::cout << "exist or not: " << exist << std::endl;
    //"database.fasta_target_0" is still not existed.
    //std::string change_name = "database.fasta_target_0";
    //if (exist) {
      //processDatabase(change_name, prsm_para_ptr, var_mod_ptr_vec);
    //}
